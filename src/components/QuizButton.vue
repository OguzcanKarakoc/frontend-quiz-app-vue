...
<template>
    <button @click="$emit('click', $event)"
        ...
        <IconContainer :text="String.fromCharCode(65 + index)" :colorClass="iconColorClass" />
        <span class="flex-grow text-start text-lg font-medium leading-full text-dark-navy dark:text-pure-white md:text-2.5xl">
            {{ text }}
        </span>
        <slot></slot>
    </button>
